...
import { linkManagementColumn } from "@/utils/columnData";
import { addDialog } from "@/components/ReDialog";
import CustomForm from "@/components/CustomForm/CustomForm.vue";
import dateUtils from "@/utils/dateUtils";
import { getLinkList, editLink, addLink, deleteLink } from "@/api/Links";
...
export const formRules = {
  avatar: [{ required: true, message: "请输入logo在线链接", trigger: "blur" }],
...
export const editRow = row => {
  addDialog({
    ...
    contentRenderer() {
      console.log(row.row);
      addFormData.value = { ...row.row, state: row.row.state === 1 };
      return (
        <CustomForm
          type="form"
