<template>
  ...
  >
    ...
    >
      ...
      >
        <h1 class="text-white text-4xl font-bold">重置密码</h1>
        <span class="text-white text-lg my-[20px]">记起密码？</span>
        <span
...
import { forgetPassword, sendCode } from "@/api/Login";
...
const getCode = (formEl: FormInstance) => {
  ...
  formEl.validateField("email", async valid => {
    if (valid) {
      const res = await sendCode({ email: formData.email, type: 1 });
